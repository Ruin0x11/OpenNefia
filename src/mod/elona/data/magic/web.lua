			...
			while i < attempts do
				...
				if success then
					Mef.create(
						...
						x,
						y,
						{ duration = opts.mef_turns(params.power), power = opts.mef_power(params.power) }
					)
...
make_web({
	...
	spread = 3,
	attempts = function(p)
		return 2 + Rand.rnd(p / 50 + 1)
	end,
	mef_id = "elona.web",
	mef_turns = function(p)
		return -1
	end,
	mef_power = function(p)
		return p * 2
	end,
})
