...
data:add({
	...
	cast = function(self, params)
		...
		if target:calc("quality") >= Enum.Quality.Great or target:calc("is_precious") or target:has_any_roles() then
...
data:add({
	...
	cast = function(self, params)
		...
		Input.query_inventory(
			source,
			"elona.inv_take_four_dimensional_pocket",
			{ container = pocket },
			"elona.container_four_dimensional_pocket"
		)
	...
	for _ = 1, passes do
		for _, x, y in map:iter_tiles() do
			if is_cursed then
				...
				if
					dist < 7
					or Rand.rnd(power + 1) > Rand.rnd(dist * 8 + 1)
					or params.curse_state == Enum.CurseState.Blessed
				then
					reveal_cb(x, y, map)
				end
			end
...
data:add({
	...
	cast = function(self, params)
		...
		local filter = function(item)
			if
				not Item.is_alive(item)
				or item.curse_state == Enum.CurseState.Normal
				or item.curse_state == Enum.CurseState.Blessed
			then
				return false
			end
	...
	cast = function(self, params)
		...
		for i = 1, times do
			...
			local item = Item.create(
				...
				source.x,
				source.y,
				{ level = level, quality = quality, amount = amount, no_stack = true },
				map
			)
			if item then
...
data:add({
	...
	calc_mp_cost = function(skill_entry, chara)
