...
local function is_in_screen(tx, ty)
	...
	return sy >= 0 and sy <= msg_y and sx >= 0 and sx <= Draw.get_width()
end
	...
	data:add({
		...
		cast = function(self, params)
			...
			for i = 0, 19 do
				...
				if
					i <= #positions
					or (map:is_in_bounds(tx, ty) and map:can_see_through(tx, ty) and is_in_screen(tx, ty))
				then
	...
	data:add({
		...
		cast = function(self, params)
			...
			if source:is_in_fov() then
				local cb =
					Anim.ranged_attack(sx, sy, tx, ty, "elona.item_projectile_magic_arrow", color, "base.arrow1", sound)
				Gui.start_draw_callback(cb)
			end
...
data:add({
	...
	cast = function(self, params)
		...
		if
			Rand.rnd(target:skill_level("elona.stat_perception"))
				> Rand.rnd(source:skill_level("elona.stat_dexterity") * 4)
				...
				for _, pos in ipairs(positions) do
					...
					if map:has_los(sx, sy, tx, ty) and (sx ~= tx or sy ~= ty) then
						...
						if chara then
							...
							chara:damage_hp(damage, params.source, {
								element = element_id,
								element_power = params.element_power,
								message_tense = tense,
								no_attack_text = true,
...
make_breath("darkness", 605, 15, 7, 8, 10)
make_breath("chaos", 606, 15, 7, 8, 10)
make_breath("sound", 607, 15, 7, 8, 10)
make_breath("nether", 608, 15, 7, 8, 10)
make_breath("nerve", 609, 15, 7, 8, 10)
make_breath("poison", 610, 15, 7, 8, 10)
make_breath("mind", 611, 15, 7, 8, 10)
...
make_summon({
	...
	count = 3,
	power = function(p, l, caster)
		return 2 + Rand.rnd(18)
	end,
	filter = function(p, l)
		return { tag_filters = { "cat" } }
	end,
})
...
make_summon({
	...
	count = 3,
	power = function(p, l, caster)
		return 5 + Rand.rnd(12)
	end,
	filter = function(p, l)
		return { tag_filters = { "yeek" } }
	end,
})
...
make_summon({
	...
	count = 3,
	power = function(p, l, caster)
		return 15 + Rand.rnd(8)
	end,
	filter = function(p, l)
		return { tag_filters = { "pawn" } }
	end,
})
...
make_summon({
	...
	count = 3,
	power = function(p, l, caster)
		return 15 + Rand.rnd(15)
	end,
	filter = function(p, l)
		return { tag_filters = { "fire" } }
	end,
})
...
make_summon({
	...
	power = summon_power_default,
	filter = function(p, l)
		return { id = "elona.younger_sister" }
	end,
...
local function do_mutation(source, target, curse_state, times, no_negative_traits)
	...
	local candidates = data["base.trait"]
		:iter()
		:filter(function(t)
...
local function make_gain_ally(opts)
	data:add({
		...
		cast = function(self, params)
			...
			local chara = Charagen.create(
				source.x,
				source.y,
				{ level = level, quality = quality, id = chara_id, tag_filters = tag_filters, level_scaling = "fixed" },
				map
			)
