...
data:add({
	...
	on_open = open_chest(function(filter, item, gen_iteration)
...
data:add({
	...
	on_open = open_chest(nil, nil, function(self, params)
		Effect.modify_karma(params.chara, -8)
	end),
	-- <<<<<<<< shade2/action.hsp:1024 	if iID(ri)=idSuitcase	: modKarma pc,-8 ..
})
...
data:add({
	...
	on_open = open_chest(
		...
		function(self, params)
			Effect.modify_karma(params.chara, -4)
		end
	),
...
data:add({
	...
	on_open = function(self, params)
		...
		Input.query_inventory(
			...
			{ container = self, params = { query_leftover = true } },
			nil
		)
...
data:add({
	...
	on_open = function(self, params)
		...
		Input.query_inventory(
			...
			{ params = { container_item = self }, container = inv },
			"elona.food_container"
		)
		-- <<<<<<<< shade2/action.hsp:932 			if invFile=roleFileFreezer : invContainer=15:el ..
...
data:add({
	...
	on_open = function(self, params)
		Input.query_inventory(
			params.chara,
			"elona.inv_take_food_container",
			{ params = { container_item = self }, container = self.inv },
			"elona.food_container"
		)
