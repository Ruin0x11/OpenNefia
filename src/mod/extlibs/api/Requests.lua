...
local multipart_post = require("mod.extlibs.internal.multipart_post")

local http_socket = require('socket.http')
local https_socket = require('ssl.https')
local url_parser = require('socket.url')
local ltn12 = require('ltn12')
local Json = require('mod.extlibs.api.Json')
local Xml = require('mod.extlibs.api.Xml')
local Md5 = require('mod.extlibs.api.Md5')
...
function _requests.make_request(request)
  ...
  response.xml = function () return Xml.parse(response.text) end
...
function _requests.create_header(request)
  ...
  if request.multipart then
     _requests.set_multipart(request)
  end
end
...
function _requests.set_multipart(request)
   table.merge(request, multipart_post.gen_request(request.multipart))
end
