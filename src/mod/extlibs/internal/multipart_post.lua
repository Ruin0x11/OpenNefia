...
local multipart_post = {}

multipart_post.CHARSET = "UTF-8"
multipart_post.LANGUAGE = ""
...
local function section_header(r, k, extra)
    ...
    if extra.filename then
        ...
        tprintf(
            r, "; filename*=%s'%s'%s",
            multipart_post.CHARSET, multipart_post.LANGUAGE, url.escape(extra.filename)
        )
    end
...
multipart_post.set_ltn12_blksz = set_ltn12_blksz
...
multipart_post.source = source

function multipart_post.gen_request(t)
...
function multipart_post.encode(t, boundary)
