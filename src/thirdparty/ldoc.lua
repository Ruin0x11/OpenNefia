...
local global = require 'thirdparty.ldoc.builtin.globals'
local markup = require 'thirdparty.ldoc.markup'
local parse = require 'thirdparty.ldoc.parse'
...
function ldoc.ldoc(target_dir, args, config)
   Item.warnings = {}
   ...
   local ldoc_dir = target_dir:gsub('[^/\\]+$','')
   ...
   local function read_ldoc_config (fname, args)
      -- use default config if passed as an argument
      if config then
         return config
      end
   ...
   if args.file == '.' then
      local err
      config_dir,err = read_ldoc_config(args.config, args)
   ...
   local function process_file (f, flist, args)
   ...
   local function reorder_module_file (args)
   ...
   if type(args.file) == 'table' then
      ...
      process_all_files(args.file, args)
   ...
   elseif path.isdir(args.file) then
      ...
      if not config_dir then
         ...
         if #config_files > 0 then
            config_dir = read_ldoc_config(config_files[1], args)
   ...
   for mod in module_list:iter() do
      if not interactive and not args.module then -- no point if we're just showing docs on the console
   ...
   local html = require 'thirdparty.ldoc.html'
...
function ldoc.dump_file(file, config)
   config = config or {}
   local new_config = {}

   for k, v in pairs(config) do
      if ldoc_contents[k] then
         new_config[k] = v
      end
   end
...
if interactive then
   local args = lapp(usage)
   ldoc.ldoc(arg[0], args)

   if args.fatalwarnings and #Item.warnings > 0 then
      os.exit(1)
   end
